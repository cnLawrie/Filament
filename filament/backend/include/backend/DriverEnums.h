...
/**
 * Defines a view port, that is the origin and extend of the clip-space.
...
struct Viewport {
    ...
    //! get the right coordinate in window space of the view port
    int32_t right() const noexcept { return left + width; }
    //! get the top coordinate in window space of the view port
...
enum class SamplerWrapMode : uint8_t {
    CLAMP_TO_EDGE,      //!< clamp-to-edge. The edge of the texture extend to infinity.
...
enum class BlendEquation : uint8_t {
    ...
    SUBTRACT,               //!< the fragment is substracted from the color buffer
    REVERSE_SUBTRACT,       //!< the color buffer is substracted from the fragment
...
struct RenderPassFlags {
    /**
     * bitmask indicating which buffer to clear at the beginning of a render pass.
     * This implies discard.
     */
    TargetBufferFlags clear;

    /**
     * bitmask indicating which buffer to discard at the beginning of a render pass.
     * Discarded buffers have uninitialized content, they must be entirely drawn over or cleared.
     */
    TargetBufferFlags discardStart;

    /**
     * bitmask indicating which buffer to discard at the end of a render pass.
     * Discarded buffers' content becomes invalid, the must not be read from again.
     */
...
struct RenderPassParams {
    RenderPassFlags flags{};    //!< operations to perform on this pass buffers

    Viewport viewport{};        //!< view port for this pass
